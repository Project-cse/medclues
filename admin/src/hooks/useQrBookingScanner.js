...
/**
 * Camera QR scanner for reception check-in.
 * Prefers BarcodeDetector; falls back to jsQR on a canvas.
 */
export function useQrBookingScanner({ enabled, onCode }) {
  ...
  const emitCode = useCallback((raw) => {
    ...
    // Always forward raw so visit-summary / wrong-type QRs can show a clear error.
