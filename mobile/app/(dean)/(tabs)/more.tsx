...
const EXTRA = DEAN_DRAWER.filter((d) => !["Dashboard", "Students", "Faculty"].includes(d.label));

export default function DeanMoreScreen() {
  const { user } = useAuth();
  return (
    <PanelShell title="More" drawerHeader={{ name: user?.name ?? "Dean", subtitle: "Menu" }} drawerItems={DEAN_DRAWER}>
