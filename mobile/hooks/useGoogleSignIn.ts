...
export function useGoogleSignIn(onSuccess?: () => void) {
  ...
  const [request, , promptAsync] = Google.useIdTokenAuthRequest(
    ...
    { scheme: "medclues", path: "oauth2redirect", preferLocalhost: false }
  );
  ...
  const signIn = useCallback(async (): Promise<{
    ...
    if (isAndroidUsingWebClientFallback()) {
      return {
        ok: false,
        error:
          "Android needs a separate Google OAuth client. Add EXPO_PUBLIC_GOOGLE_ANDROID_CLIENT_ID to mobile/.env — see docs/mobile/GOOGLE_SIGNIN.md",
      };
    }
    if (!request) {
    ...
    try {
      ...
      return {
        ok: false,
        error:
          "Google blocked sign-in (Error 400). Add Android OAuth client ID in mobile/.env — see docs/mobile/GOOGLE_SIGNIN.md",
      };
    } catch (e) {
