...
**Display name (Android):** MedClues (launcher label in `AndroidManifest.xml`)
...
| Backend deploy guide | [../docs/BACKEND_GITHUB_DEPLOY.md](../docs/BACKEND_GITHUB_DEPLOY.md) |
| Telegram bot (backend) | [../docs/backend/TELEGRAM_BOT.md](../docs/backend/TELEGRAM_BOT.md) |
...
| Deep link scheme | `medclues://dashboard` (primary; also accepts `mediclues` / `medichain`) |
...
| `/dashboard` | **Open app home** button (`medclues://dashboard`) |
...
Telegram **Open App Home** and similar buttons use the custom scheme (primary `medclues://`; legacy aliases `mediclues://` and `medichain://` are accepted):

```
medclues://dashboard              → app home (/dashboard)
medclues://open/appointments      → appointments tab
medclues://open/records           → health records
medclues://open/doctors           → doctor list
medclues://open/profile           → profile
medclues://payment?cancelled=1    → appointments (payment cancelled)
medclues://payment?failed=1       → appointments (payment failed)
medclues://payment                → appointments (payment return)
```
...
| `android/app/src/main/AndroidManifest.xml` | `medclues` (+ legacy) intent filters |

**Test deep link (USB Android):**

```bash
adb shell am start -a android.intent.action.VIEW -d "medclues://dashboard" com.medichain.medichain_mobile
```

> Deep links open the **native APK only**. Chrome/web does not handle `medclues://` — use normal navigation in the web build.
...
| `app_deep_link_service.dart` | `medclues://` → router |
...
| Display name (launcher) | MedClues (`AndroidManifest.xml`) |
...
- Test with: `adb shell am start -a android.intent.action.VIEW -d "medclues://dashboard" com.medichain.medichain_mobile`
...
|----------|----------|
| Monorepo overview | [../README.md](../README.md) |
| Backend deploy | [../docs/BACKEND_GITHUB_DEPLOY.md](../docs/BACKEND_GITHUB_DEPLOY.md) |
| Backend security steps | [../docs/backend/SECURITY_STEP1.md](../docs/backend/SECURITY_STEP1.md) |
| Agora video | [../docs/backend/AGORA_VIDEO.md](../docs/backend/AGORA_VIDEO.md) |
| Telegram bot | [../docs/backend/TELEGRAM_BOT.md](../docs/backend/TELEGRAM_BOT.md) |
| Phone API testing | [../docs/backend/README_PHONE.md](../docs/backend/README_PHONE.md) |
| Phone verification setup | [../docs/mobile/PHONE_VERIFICATION_SETUP.md](../docs/mobile/PHONE_VERIFICATION_SETUP.md) |
