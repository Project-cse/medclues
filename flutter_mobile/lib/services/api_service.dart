...
class ApiService {
  ...
  static ApiService create(StorageHelper storage,
      {OnUnauthorized? onUnauthorized}) {
    final dio = Dio(
      BaseOptions(
    ...
    // Transient network / 5xx retry (max 2), with backoff
    dio.interceptors.add(
      InterceptorsWrapper(
        onError: (error, handler) async {
          final status = error.response?.statusCode;
          final type = error.type;
          final retryable = type == DioExceptionType.connectionTimeout ||
        ...
        if (status == 401 && !isAuthPath && !isRetry) {
          ...
          if (refreshed) {
            try {
              ...
              final newToken =
                  await storage.getAccessToken() ?? service._memoryToken;
      ...
      case DioExceptionType.badResponse:
        ...
        if (code == 404) {
          return AppException('Not found',
              type: AppExceptionType.validation, statusCode: 404);
        }
        if (code != null && code >= 500) return AppException.server(msg);
        return AppException(msg ?? 'Request failed',
            type: AppExceptionType.validation, statusCode: code);
      default:
