...
class HealthRecordService {
  ...
  Future<List<HealthRecordItem>> fetchAll() async {
    ...
    final data = res.data ?? {};
    assertSuccess(data);
    final out = <HealthRecordItem>[];
    for (final raw in unwrapList(data, ['records', 'healthRecords', 'data'])) {
      try {
        out.add(HealthRecordItem.fromJson(raw));
      } catch (_) {
        // Skip malformed records so one bad row cannot blank the Records tab.
      }
    }
    return out;
  }
