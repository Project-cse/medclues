...
class BookingPatientSelectorScreen extends ConsumerStatefulWidget {
  const BookingPatientSelectorScreen(
  ...
  @override
  ConsumerState<BookingPatientSelectorScreen> createState() =>
      _BookingPatientSelectorScreenState();
}

class _BookingPatientSelectorScreenState
    extends ConsumerState<BookingPatientSelectorScreen> {
  ...
  void _ensureLocalizedDefaults(AppLocalizations l10n) {
    ...
    if (_relationship.isEmpty)
      _relationship = LocalizedFormOptions.relationships(l10n).first;
  }
  ...
  void initState() {
    ...
    Future.microtask(
        () => prefetchDoctorSchedule(ref, widget.doctorId, mode: mode));
  }
  ...
  Future<void> _bookForOthers() async {
    final l10n = context.l10n;
    ...
    final patient = PatientBookingInfo(
      ...
      relationship:
          LocalizedFormOptions.relationshipToStorage(_relationship, l10n),
      isSelf: false,
    );
    if (_saveForLater) {
      try {
        await ref.read(savedProfileServiceProvider).create(patient.toJson());
  ...
  void _bookSavedProfile(SavedProfile profile) {
    ref.read(bookingPatientProvider.notifier).state =
        profile.toPatientBookingInfo();
    context.push(_bookingPath);
  }
            ...
            child: Container(
              decoration: BoxDecoration(
                ...
                borderRadius: const BorderRadius.vertical(
                    top:
                        Radius.circular(PremiumHealthcareTheme.sheetTopRadius)),
                boxShadow: [
                  BoxShadow(
              ...
              child: doctorAsync.isLoading
                  ...
                  : _showPatientForm
                      ? _PatientDetailsForm(
                          ...
                          onSaveForLaterChanged: (value) =>
                              setState(() => _saveForLater = value),
                          onBack: () => setState(() {
                            _showPatientForm = false;
                            _showSavedProfiles = true;
                          }),
                      ...
                      : _showSavedProfiles
                          ? _SavedPatientPicker(
                              ...
                              onSelected: _bookSavedProfile,
                              onNew: () => setState(() {
                                _showSavedProfiles = false;
                                _showPatientForm = true;
                              }),
                              onBack: () =>
                                  setState(() => _showSavedProfiles = false),
                            )
                          : _WhoIsItFor(
                              l10n: l10n,
                              onMyself: _bookForMe,
                              onSomeoneElse: () =>
                                  setState(() => _showSavedProfiles = true),
                            ),
            ),
          ),
        ],
...
class _SavedPatientPicker extends StatelessWidget {
  const _SavedPatientPicker({
  ...
  final AsyncValue<List<SavedProfile>> profiles;
  final ValueChanged<SavedProfile> onSelected;
      ...
      children: [
        ...
        profiles.when(
          ...
          error: (_, __) => const Text(
              'Saved profiles could not be loaded. You can enter new details.'),
          data: (items) => Column(
            children: items
                .map(
                  (profile) => Card(
                    child: ListTile(
                      ...
                      subtitle:
                          Text('${profile.relationship} • Age ${profile.age}'),
        ...
        OutlinedButton.icon(
          ...
          label: const Text('Enter details for someone new'),
        ),
      ],
    );
  }
...
class _PatientDetailsForm extends StatelessWidget {
  const _PatientDetailsForm({
    ...
    required this.onSaveForLaterChanged,
  });
  ...
  final ValueChanged<bool> onSaveForLaterChanged;
              ...
              child: Column(
                ...
                children: [
                  ...
                  CheckboxListTile(
                    ...
                    value: saveForLater,
                    onChanged: (value) => onSaveForLaterChanged(value ?? false),
                    title: const Text('Save this profile for next time'),
