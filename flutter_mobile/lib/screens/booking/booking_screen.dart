...
class BookingScreen extends ConsumerStatefulWidget {
  const BookingScreen(
      {super.key, required this.doctorId, this.preferOnline = false});
...
class _BookingScreenState extends ConsumerState<BookingScreen> {
  ...
  late final List<
      ({String label, int dayNum, String slotDate, String monthShort})> _week;
  Timer? _slotRefreshTimer;

  PatientBookingInfo get _patient =>
      ref.read(bookingPatientProvider) ??
      PatientBookingInfo(name: 'Patient', isSelf: true);
  ...
  Future<void> _completeBooking({
    ...
    final bookingId =
        result['bookingId']?.toString() ?? result['booking_id']?.toString();
    final publicId =
        result['publicId']?.toString() ?? result['public_id']?.toString();
    ...
    if (apptId.isNotEmpty) {
      ref.read(lastBookedAppointmentIdProvider.notifier).state = apptId;
    }
          ...
          children: [
            ...
            ListTile(
              ...
              subtitle: Text('Attach a report you already uploaded',
                  style: GoogleFonts.inter(
                      fontSize: 12,
                      color: PremiumBookingTheme.textSecondary(ctx))),
          ...
          builder: (ctx, scrollCtrl) => Consumer(
            builder: (ctx, ref2, _) {
              ...
              return records.when(
                ...
                data: (list) {
                  final withFiles =
                      list.where((r) => r.files.isNotEmpty).toList();
                  return ListView(
                    ...
                    children: [
                      ...
                      if (withFiles.isEmpty)
                        ...
                        ...withFiles.map((r) {
                          final fileName =
                              r.files.first['fileName']?.toString() ?? r.title;
                          return Container(
  ...
  Future<void> _attachExistingReport(HealthRecordItem record) async {
    ...
    try {
      ...
      final fileName =
          record.files.first['fileName']?.toString() ?? 'report.pdf';
      ...
      if (!mounted) return;
      setState(() {
        _reportFile = PlatformFile(
            name: dl.fileName, size: dl.bytes.length, bytes: dl.bytes);
  ...
  Future<void> _submitBooking(DoctorModel doctor) async {
    if (ref.read(bookingPatientProvider) == null) {
      context.go(_patientSelectorPath);
      return;
    }
    ...
    final availableSymptoms =
        SpecializationSymptoms.forSpecialization(doctor.specialization);
    ...
    try {
      if (_requiresRazorpay) {
        ...
        final fee =
            isVideo ? doctor.videoConsultationFee : doctor.consultationFee;
  ...
  Future<Map<String, dynamic>> _waitForPaymentCompletion(
      PaymentOrderResult order) async {
    final paymentService = ref.read(paymentServiceProvider);
    final l10n = context.l10n;
                ...
                return AlertDialog(
                  ...
                  content: Column(
                    ...
                    children: [
                      ...
                      Text(statusLabel),
                      const SizedBox(height: 12),
                      Text(l10n.paymentOrderLabel(order.orderId),
                  ...
                  actions: [
                    ...
                    FilledButton(
                      onPressed: () {
                        if (!confirmNowCompleter.isCompleted)
                          confirmNowCompleter.complete();
                      },
      ...
      while (DateTime.now().isBefore(deadline)) {
        ...
        if (!firstCheck) {
          await Future.any<void>([
            ...
            if (!confirmNowConsumed) confirmNowCompleter.future,
          ]);
        }
        firstCheck = false;
        if (confirmNowCompleter.isCompleted) {
          confirmNowConsumed = true;
        }
        if (cancelCompleter.isCompleted) {
          throw Exception(l10n.paymentCancelled);
        }
        ...
        try {
          final result =
              await _tryConfirmPaidOrder(paymentService, order.orderId);
          return result;
        } catch (e) {
          final msg = e.toString().replaceFirst('Exception: ', '');
          if (msg.contains('Payment failed') ||
              msg.contains('Payment cancelled')) {
            rethrow;
          }
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(context.l10n.paymentOrderLabel(order.orderId),
                style: const TextStyle(fontSize: 12)),
            TextField(
                controller: paymentId,
                decoration:
                    InputDecoration(labelText: context.l10n.paymentPaymentId)),
            TextField(
                controller: signature,
                decoration:
                    InputDecoration(labelText: context.l10n.paymentSignature)),
          ],
        ),
        actions: [
          TextButton(
          ...
          TextButton(
              ...
              child: Text(context.l10n.paymentVerify)),
        ],
      ),
    );
    if (result != true || paymentId.text.isEmpty || signature.text.isEmpty)
      return null;
  ...
  void _selectDay(int index) {
    ...
    if (_dateScroll.hasClients) {
      final offset =
          (index * 82.0).clamp(0.0, _dateScroll.position.maxScrollExtent);
      _dateScroll.animateTo(offset,
  ...
  Future<void> _openCalendarPicker() async {
    ...
    final idx = _week.indexWhere(
      (d) =>
          d.dayNum == picked.day &&
          d.monthShort == DateFormat('MMM').format(picked),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final pageTitle = widget.preferOnline
        ? l10n.bookingBookVideoConsult
          ...
          return Column(
            children: [
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    PremiumBookingSectionHeader(
                      ...
                      trailing:
                          PremiumViewCalendarAction(onTap: _openCalendarPicker),
                    ),
                    const SizedBox(height: 14),
                    SizedBox(
                      height: 108,
                    ...
                    const SizedBox(height: PremiumBookingTheme.sectionGap),
                    PremiumBookingSectionHeader(
                      title: widget.preferOnline
                          ? l10n.bookingVideoSlots
                          : l10n.bookingOpdSlots,
                    ...
                    scheduleAsync.when(
                      ...
                      data: (schedule) {
                        ...
                        final selectedStillValid = _selectedSlot == null ||
                            slots.any(
                              (s) =>
                                  s.time == _selectedSlot!.time && s.available,
                            );
                      ...
                      decoration: InputDecoration(
                        ...
                        border: OutlineInputBorder(
                          ...
                          borderSide: BorderSide(
                              color: PremiumBookingTheme.border(context)),
                        ),
              ...
              PremiumBookAppointmentCta(
                ...
                loading: _booking,
                onPressed:
                    canBook && !_booking ? () => _submitBooking(doctor) : null,
              ),
            ],
          );
        },
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.description_outlined,
                size: 18, color: PremiumBookingTheme.accentBlue),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.insert_drive_file_outlined,
                            size: 16, color: PremiumBookingTheme.accentBlue),
