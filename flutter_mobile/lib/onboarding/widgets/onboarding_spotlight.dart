...
class _OnboardingSpotlightState extends State<OnboardingSpotlight>
  ...
  static const _estTooltipH = 200.0;
  ...
  void initState() {
    super.initState();
    _enter = AnimationController(vsync: this, duration: const Duration(milliseconds: 280));
  ...
  Future<void> _syncHole() async {
    if (!mounted) return;
    final key = widget.step.target.anchorKey;
    ...
    // Wait for list layout / animate() to settle, then measure in overlay space.
    await Future<void>.delayed(const Duration(milliseconds: 120));
    if (!mounted) return;
    for (var i = 0; i < 4; i++) {
      final hole = widget.step.target.resolveInOverlay(context);
      if (hole != null) {
        setState(() => _resolvedHole = hole);
        return;
      }
    ...
    setState(() {
      _resolvedHole = widget.step.target.fallbackRect(MediaQuery.sizeOf(context));
    });
  }
  ...
  Rect _anchorFor(Size size) =>
      _resolvedHole ?? widget.step.target.rectFor(size, overlayContext: context);
...
class _SpotlightPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Soft scrim with a tight hole — single thin ring, no heavy outline stack.
    final hole = RRect.fromRectAndRadius(
      anchor.inflate(1.5),
      Radius.circular(anchorRadius),
    );

    canvas.saveLayer(full, Paint());
    canvas.drawRect(
      full,
      Paint()..color = const Color(0xFF0B1220).withValues(alpha: 0.48),
    );
    ...
    canvas.drawRRect(
      hole,
      Paint()
        ..color = PremiumHealthcareTheme.secondaryBlue.withValues(alpha: 0.9)
  ...
  bool shouldRepaint(covariant _SpotlightPainter old) =>
      old.anchor != anchor || old.anchorRadius != anchorRadius;
}
